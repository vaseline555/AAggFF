...
class AaggffServer(FedavgServer):
    def __init__(self, **kwargs):
        super(AaggffServer, self).__init__(**kwargs)
    ...
    def update(self):
        ...
        # Server Update #
        #################
        # See pseudocodes in p.32 - 33 of the paper
        # output base mixing coefficients for comparison (i.e., n_i / sum_i n_i)
        ...
        ## update regret - eq. (4)
        l_t = r_t.dot(self.mix_coefs).log1p().mul(-1)

        ## log regret - eq. (5)
        self.cum_global_obj = self.cum_global_obj.add(l_t)
        self.results[self.round]['global_obj'] = {'global_obj': l_t.item()}
        self.writer.add_scalars('Cumulative Global Objective Value', {'Cumulative Global Objective Value': self.cum_global_obj.item()}, self.round)
        logger.info(f'[{self.args.algorithm.upper()}] [{self.args.dataset.upper()}] [Round: {str(self.round).zfill(4)}] [SERVER]\t- Cumulative Server Objective Value: {self.cum_global_obj.item():.4f}')
        
        ## transform range of return vectors to be bounded using a CDF - eq. (8)
        ### See also Section B
        if self.args.fair_const == 0: # Weibull CDF (shape=2, scale=1; shape > 1 where outliers increased through training)
            ...
            r_t[selected_ids] = torch.erfc((r_t[selected_ids].div(r_t[selected_ids].mean())).sub(1).div(2**0.5)).add(1).mul(0.5)
        ...
        ## change bound to achieve O(1) Lipschitz continuity - Section B.2
        if self.args.C < 1:
        ...
        if self.args.C == 1: ### cross-silo setting - Algorithm 2 in p.32
            #### return gradient
        ...
        else: ### cross-device setting - Algorithm 3 in p.33
        ...
        ## renormalize mixing coefficients only for selected clients to be used for model aggregation - eq. (95)
