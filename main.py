...
if __name__ == "__main__":
    ...
    parser.add_argument('--fair_const', help='fairness constant;\
        `rho` for `AAggFF`,\
        or `q` for `q-FedAvg`,\
        or `epsilon` for `FedMGDA`,\
        or `tau` for `TERM`,\
        or `M` for `PropFair`,\
        or `theta` for `FedMDFG`', type=float, choices=[Range(0., float('inf'))], default=1.)
    parser.add_argument('--algorithm', help='federated learning algorithm to be used', type=str,
        choices=['fedavg', 'fedsgd', 'fedprox', 'fedavgm', 'fedadam', 'fedyogi', 'fedadagrad', 'afl', 'qfedavg', 'fedmgda', 'term', 'propfair', 'fedmdfg', 'aaggff', 'aaggffprox', 'aaggffadam', 'aaggffyogi', 'aaggffadagrad'], 
        required=True
    )
